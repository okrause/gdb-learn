...
/*
 * gradebook.c — GDB tutorial: inspect structs, pointers, and a real bug.
 *
 * Build (debug symbols, no optimization so lines match source):
 *   gcc -g -O0 -o gradebook gradebook.c
 *
 * Then:
 *   gdb ./gradebook
 *
 * Intentional bug: compute_average() divides by the wrong count, so
 * averages are too low. Use GDB to watch n, sum, and the scores array
 * and find it.
 */
